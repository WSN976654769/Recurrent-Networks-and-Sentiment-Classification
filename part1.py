...
class rnn(torch.nn.Module):
    ...
    def rnnCell(self, input, hidden):
        ...
        """
        # ht = f(wh*ht-1+wx*x+b)
        h_next = self.ih(input)+self.hh(hidden)
        tanh = torch.nn.Tanh()
        return tanh(h_next)
...
class rnnSimplified(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # self.net = None
        # self.net = rnn()
        self.net = torch.nn.RNN(input_size=64,hidden_size=128,batch_first=False)
...
def conv(input, weight):
    ...
    """
    conv = torch.nn.functional.conv1d(torch.autograd.Variable(input),torch.autograd.Variable(weight))
    return conv
